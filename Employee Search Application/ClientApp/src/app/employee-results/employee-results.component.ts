...
export class EmployeeResultsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.dataSource.filterPredicate = (data: Employee, filter: string) => {
      ...
      if (
        criteria.firstName &&
        data.firstName.toLowerCase().includes(criteria.firstName.toLowerCase())
      ) {
        match = true;
      }
      if (
        criteria.lastName &&
        data.lastName.toLowerCase().includes(criteria.lastName.toLowerCase())
      ) {
        match = true;
      }
      if (
        criteria.startDate &&
        data.endDate.isSameOrAfter(criteria.startDate) &&
        criteria.endDate &&
        data.startDate.isSameOrBefore(criteria.endDate)
      ) {
        match = true;
      }
      return match;
